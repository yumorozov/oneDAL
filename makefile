...
-Zl    := $(-Zl.$(COMPILER))
-DEBC  := $(if $(REQDBG),$(-DEBC.$(COMPILER)) -DDEBUG_ASSERT)
...
TBBDIR.libia   := $(TBBDIR.2)/lib$(if $(or $(OS_is_mac),$(OS_is_fbsd)),,/$(_IA)$(if $(OS_is_win),/vc_mt,/gcc4.4))
TBBDIR.soia    := $(TBBDIR.2)/$(if $(OS_is_win),../redist,lib)$(if $(or $(OS_is_mac),$(OS_is_fbsd)),,/$(_IA)/$(if $(OS_is_win),tbb/vc_mt,gcc4.4))
...
daal_dbg:
	@echo "1" "!$(USECPUS.out.defs.filter)!"
