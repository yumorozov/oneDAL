...
services::Status LBFGSKernel<algorithmFPType, defaultDense, cpu>::compute(HostAppIface* pHost,
    NumericTable* correctionPairsInput, NumericTable* correctionIndicesInput,
    ...
    OptionalArgument *optionalArgumentResult, Parameter *parameter, engines::BatchBase &engine)
{
    ...
    const bool useWolfeConditions = (batchSize == nTerms &&
                                     correctionPairBatchSize == nTerms &&
                                     L == 1);
...
algorithmFPType LBFGSTask<algorithmFPType, cpu>::lineSearch(algorithmFPType* x, NumericTablePtr &ntValue, NumericTablePtr &ntGradient, algorithmFPType* dx, sum_of_functions::BatchPtr &gradientFunction, bool& continueSearch)
{
    ...
    while(daal::internal::Math<algorithmFPType,cpu>::sFabs(stepLength) > minStepLength)
    {
        ...
        if (fvNew.get()[0] - oldFv <= c1 * stepLength * term1)
        {
            ...
            if(daal::internal::Math<algorithmFPType,cpu>::sFabs(fgNewXdn)
                <= c2 * daal::internal::Math<algorithmFPType,cpu>::sFabs(term1))
            {
                break;
            }
        }
    ...
    NumericTable *correctionPairsResult, size_t nTerms, size_t batchSize, size_t correctionPairBatchSize)
{
    ...
    const bool useWolfeConditions = ( batchSize == parameter->function->sumOfFunctionsParameter->numberOfTerms &&
       correctionPairBatchSize == parameter->function->sumOfFunctionsParameter->numberOfTerms &&
       parameter->L == 1);
    if(useWolfeConditions)
    {
