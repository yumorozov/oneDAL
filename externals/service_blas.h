...
struct Blas
{
    ...
    static void xgemm(const char *transa, const char *transb, const SizeType *p, const SizeType *ny, const SizeType *n, const fpType *alpha, const fpType *a,
              const SizeType *lda, const fpType *y, const SizeType *ldy, const fpType *beta, fpType *aty, const SizeType *ldaty)
    {
        _impl<fpType,cpu>::xgemm(transa, transb, p, ny, n, alpha, a, lda, y, ldy, beta, aty, ldaty);
    }

    static void xxgemm(const char *transa, const char *transb, const SizeType *p, const SizeType *ny, const SizeType *n, const fpType *alpha, const fpType *a,
              const SizeType *lda, const fpType *y, const SizeType *ldy, const fpType *beta, fpType *aty, const SizeType *ldaty)
    {
        _impl<fpType,cpu>::xxgemm(transa, transb, p, ny, n, alpha, a, lda, y, ldy, beta, aty, ldaty);
    }

    static void xsymm(const char *side, const char *uplo, const SizeType *m, const SizeType *n, const fpType *alpha, const fpType *a, const SizeType *lda,
               const fpType *b, const SizeType *ldb, const fpType *beta, fpType *c, const SizeType *ldc)
    {
    ...
    static void xgemv(const char *trans, const SizeType *m, const SizeType *n, const fpType *alpha, const fpType *a, const SizeType *lda, const fpType *x,
               const SizeType *incx, const fpType *beta, fpType *y, const SizeType *incy)
    {
        _impl<fpType,cpu>::xgemv(trans, m, n, alpha, a, lda, x, incx, beta, y, incy);
    }

    static void xxgemv(const char *trans, const SizeType *m, const SizeType *n, const fpType *alpha, const fpType *a, const SizeType *lda, const fpType *x,
               const SizeType *incx, const fpType *beta, fpType *y, const SizeType *incy)
    {
    ...
    static services::Status xgemm_blocked( const char         *transa,
                                           const char         *transb,
                                           const SizeType     *na,
                                           const SizeType     *nb,
                                           const SizeType     *cols,
                                           const fpType       *alpha,
                                           const NumericTable *ta,
                                           const SizeType     *lda,
                                           const NumericTable *tb,
                                           const SizeType     *ldb,
                                           const fpType       *beta,
                                           NumericTable       *tc,
                                           const SizeType     *ldc,
                                           int                a_req_blocksize = -1,
                                           int                b_req_blocksize = -1
                                         )
    {
        SafeStatus safeStat;
        ...
        else                    {  a_lastblocksize = a_blocksize;  }
        if(b_lastblocksize != 0){  b_blocknum++;  }
        else                    {  b_lastblocksize = b_blocksize;  }
        ...
        daal::threader_for( b_blocknum, b_blocknum, [&](SizeType b_block)
        {
            ...
            ReadRows<fpType, cpu> mtb(*const_cast<NumericTable *>(tb), b_block*b_blocksize, b_cursize); DAAL_CHECK_BLOCK_STATUS_THR(mtb);
            const fpType *b = const_cast<fpType *>(mtb.get());

            /* Get pointer to write resulted rows */
            WriteOnlyRows<fpType, cpu> mtc(tc, b_block*b_blocksize, b_cursize); DAAL_CHECK_BLOCK_STATUS_THR(mtc);
            fpType *c = mtc.get();
            ...
            daal::threader_for( a_blocknum, a_blocknum, [&](SizeType a_block)
            {
                ...
                ReadRows<fpType, cpu> mta(*const_cast<NumericTable *>(ta), a_block*a_blocksize, a_cursize); DAAL_CHECK_BLOCK_STATUS_THR(mta);
                const fpType *a = const_cast<fpType *>(mta.get());

                /* Call to sequential GEMM */
                xxgemm( transa, transb,
                      &a_cursize, &b_cursize, &ab_cols,
                       alpha,
                       a, lda,
                       b, ldb,
                       beta,
                       c + a_block*a_blocksize, ldc );
            });
        });
